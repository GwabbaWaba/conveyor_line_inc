...
fn main() -> DynErrResult<ExitCode> {
    ...
    // cleans the terminal when dropped. I love ~ATH
    let _terminal_settings = TerminalSettings::setup_terminal(game.terminal().backend_mut())?;
        ...
        while let Ok(true) = event::poll(Duration::from_secs(0)) {
            ...
            #[cfg(debug_assertions)] {
                match event {
                    ...
                    // poor man's hot reload (use with cargo xrun)
...
fn setup_db(game: &GameWrapper, sql_db: rusqlite::Connection) -> DynErrResult<()> {
...
impl UserData for SqlWrapper {
    fn add_methods<'lua, M: UserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        methods.add_method("query", |lua, this, (sql_instructions, sql_args): (String, Table)| {
            ...
            lua_table_to_sql_args(sql_args, &mut buf).unwrap();
            let args = buf.iter().map(|b_ts: &Box<dyn ToSql>| &**b_ts).collect_vec();
            let mut stmt = this.prepare(&sql_instructions).unwrap();
